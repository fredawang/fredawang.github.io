...
ColReorder.prototype = {
	...
	"_fnMouseUp": function ( e, colResized)
	{
		...
		//Martin Marchetta
			...
			console.log("new width:",$(this.s.mouse.resizeElem).innerWidth());
			
			//If other columns might have changed their size, save their size too
			scrollXEnabled = this.s.dt.oInit.sScrollX === "" ? false:true;			
			console.log("SXEN",scrollXEnabled);
			if(!scrollXEnabled){
				...
				console.log(	"mywidth2", $(this.s.mouse.resizeElem).next().innerWidth() + "px");
				if(this.s.dt.aoColumns.length > nextVisibleColumnIndex)
					...
					for(i = previousVisibleColumnIndex; i > 0; i--){
						if(this.s.dt.aoColumns[i].bVisible){
							...
							console.log("WIDTH3:", $(currentColumn).innerWidth() + "px");
						}
					}
				}
			}			
			...
			if(scrollXEnabled && $('div.dataTables_scrollHead', this.s.dt.nTableWrapper) != undefined){
				...
				console.log("WRAP:",$('div.dataTables_scrollHead', this.s.dt.nTableWrapper));
				console.log($($('div.dataTables_scrollHead', this.s.dt.nTableWrapper)[0].childNodes[0].childNodes[0]).width());
			}
